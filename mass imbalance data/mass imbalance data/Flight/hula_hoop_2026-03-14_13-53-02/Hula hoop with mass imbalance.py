...
class Simulation:
    ...
    def get_R(self,iter,X,prev_X,prev_AV,prev_q,prev_u,prev_gNdot,prev_gammaF,*index_sets):
        ...
        # updated applied forces (weight with mass imbalance)
        tau=self.m*self.gr*self.d*np.cos(q[2]) 
        force = np.array([[0,-self.m*self.gr,-tau]])
        ...
        M = np.array([[self.m, 0, -self.m*self.ecc*self.R*np.sin(theta)],
                      ...
                      [-self.m*self.ecc*self.R*np.sin(theta), self.m*self.ecc*self.R*np.cos(theta),self.m*self.R**2]])

        # Residual components:
        velocity_res = np.array([u[2]**2*np.cos(q[2]),u[2]**2*np.sin(q[2]),0])*self.ecc*self.R*self.m
        ...
        Rs = np.concatenate(([M@a-force-velocity_res-Wg@lambda_g-Wgamma@lambda_gamma-WN@lambdaN-WF@lambdaF],
...
if __name__ == "__main__":
    ...
    test = Simulation(
        ...
        y0=0.18, # geometric center at t=0
        x0=0.0,
    )
    test.solve()